...
import { collection, query, getDocs, doc, updateDoc, deleteDoc, onSnapshot, orderBy, writeBatch } from "firebase/firestore";
import { handleFirestoreError, OperationType } from "../lib/firestore-errors";
import { Shield, Users, Building2, Database, Trash2, Edit2, Check, X, Search, MoreVertical, Terminal, Info, Zap } from "lucide-react";
import { toast } from "sonner";
import { motion, AnimatePresence } from "motion/react";

type Tab = "users" | "contacts" | "prompts";
  ...
  const handleSeedPrompts = async () => {
    ...
    try {
      ...
      DEFAULT_SEED_PROMPTS.forEach(p => {
        ...
        console.log(`Setting prompt: ${p.id}`);
        batch.set(ref, {
          name: p.name,
      ...
      console.error("Seeding error explicitly caught:", error);
      toast.error("Failed to seed prompts. Check console for error details.", { id: toastId });
